...
| D | Copilot endpoint — validated answers, cards, evidence, confidence, unknowns, approval requirements from fixture packets. | **DONE** — Custonllm `POST /copilot` (branch `claude/copilot-slice-b`, pushed): deterministic grounded synthesis from the packet only (no model call yet — flag-gated LLM enrichment comes later behind the same contract); `copilot` perm denies the customer role; 25 endpoint tests over the golden fixtures; Custonllm suite 720 green. |
| E | UI integration — structured cards, evidence, approval states in HyperKernel Chat; graceful failure when model/adapter unavailable. | **DONE** — `copilot-remote-adapter.js` (fail-closed: unconfigured/invalid/ungrounded replies discarded), `contract-card-renderer.js` (escape-safe cards + evidence chips + approval/unknown/degraded states), `copilot-chat-bridge.js` (phase-one routing), chat-canvas pre-empt hook that falls through to the local Executive Copilot on ANY remote failure. Endpoint activates via the `copilotEndpoint` config flag (unset = local-only, honestly absent). |
| F | Evals and telemetry — golden datasets, graders, traces, cost/latency/schema-failure/approval metrics, regression gates in CI. | **DONE (core)** — CI eval gates in both repos (`test/unit/copilot-eval-gate.test.js` + `tests/test_copilot_eval_gate.py`: systematic mutant corpora where 100% rejection IS the gate; shared invalid + degraded fixtures); client telemetry `AAA_COPILOT_TELEMETRY` (outcome/error/job counters, latency p50/p95, budget violations, PII-free); server `GET /copilot/metrics` (requests/ok/422/mismatch/per-job/approval counts + latency percentiles, diag-gated); calibration-ledger prediction per response (X-Copilot-Prediction-Id header → existing outcome/calibration flow); adapter timing on every result. Remaining for G: dashboards over these surfaces + budget alerting. |
