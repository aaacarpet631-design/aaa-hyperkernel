...
4. Save the draft. Subsequent saves update the same quote. The working form is saved on this device while typing; saved quotes appear in the existing pipeline. **Resume unfinished work** restores other unfinished forms, including incomplete measurements or missing prices. Starting another form preserves the previous work.
...
Closing a confirmation with ×, Escape, the backdrop or Cancel resolves as cancellation. Escape and backdrop dismissal affect only the top sheet; canceling approval leaves the underlying quote editor open. Only the explicit confirmation button approves the action, so a dismissed approval cannot leave the quote screen waiting indefinitely.

Only one quote editor is open at a time in a runtime. Switching forms first saves the current working copy; if that save fails, the original editor stays open. Each form has its own recovery key. The older single-slot form is preserved when upgrading. Clean saved forms are excluded from the unfinished-work list. This recovery list is local to the workspace and device. It does not provide cross-tab locking or multi-device sync.

Save and approval errors appear beside the action buttons, separately from automatic-save messages. A save that encounters a changed revision immediately offers recovery as a new draft or reopening the latest saved quote. The underlying form is inactive during a save or approval confirmation.
...
- Retains the $45-per-room cleaning floor and 1.5× stair labor rule. Larger measured cleaning areas and the configured trip minimum can increase the total. Entering a cleaning minimum below $45 now produces a validation error instead of displaying a rate the engine will silently raise.
...
- Rejects negative, non-finite and incomplete values. Clearing a material price or room/unit count produces an error instead of substituting a default. An explicit zero material price is still accepted for customer-supplied material. Receipt amounts sum to the stored total in cents.
...
- Working copies are device-local and workspace-scoped. Quote cloud mirroring retains the existing best-effort behavior; this change does not establish multi-device conflict resolution or guaranteed cloud backup. Device save confirmations do not wait for quote, audit, outcome or supervisor-score cloud mirroring. Quote mirrors are ordered per quote within a runtime, coalescing pending changes to the latest snapshot. These queues are in memory; closing the app can interrupt backup. A stalled request can delay that quote's later cloud snapshots but cannot block its local workflow.
...
The September 9 audit adds coverage for separate unfinished forms and legacy-form migration, form switching and storage failure, live revision-conflict recovery, cleared prices and quantities, the displayed cleaning minimum, stalled quote/audit/supervisor backup, ordered quote snapshots, and nested confirmation dismissal. See `docs/QUOTE_BUILDER_AUDIT.md` for the rating and remaining release limitations.
